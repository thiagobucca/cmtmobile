...
export function * getCupomUser (api, action) {
  const { usuario_id } = action
  // make the call to the api
  const apiCall = call(api.getCupomUser, usuario_id.usuario_id)
  const response = yield call(callApi, apiCall)

  // success?
  if (response.ok) {
    yield put(CupomActions.cupomUserSuccess(response.data))
  } else {
    yield put(CupomActions.cupomUserFailure(response.data))
  }
}
