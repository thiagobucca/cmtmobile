...
describe('ContatoLojaMaconica Screen Tests', () => {
  before(async () => {
  ...
  beforeEach(async () => {
    await device.reloadReactNative()
    await navigateToContatoLojaMaconicaScreen()
  })

  const navigateToContatoLojaMaconicaScreen = async () => {
    await expect(element(by.id('launchScreen'))).toBeVisible()
    await element(by.id('menuButton')).tap()
    await element(by.id('entitiesDrawerButton')).tap()
    await element(by.id('contatoLojaMaconicaEntityScreenButton')).tap()
  }

  it('should allow you to create an entity', async () => {
    await expect(element(by.id('contatoLojaMaconicaScreen'))).toBeVisible()
